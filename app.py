...
.main-title { font-size: 26px; font-weight: 700; text-align: center; color: #333333; margin-bottom: 20px; }
.question-box { background-color: #f8f9fa; padding: 22px; border-radius: 15px; border: 1px solid #e9ecef; margin-bottom: 20px; }
.question-text { font-size: 19px; font-weight: 700; color: #212529; margin: 0; line-height: 1.5; }
.persona-card { background-color: #e7f5ff; padding: 20px; border-radius: 15px; border-left: 5px solid #1c7ed6; margin-bottom: 20px; }
.portfolio-card { background-color: #f3f0ff; padding: 18px; border-radius: 12px; border: 1px solid #d0bfff; margin-bottom: 20px; }
.product-card { background-color: #ffffff; padding: 18px; border-radius: 12px; border: 1px solid #dee2e6; margin-bottom: 16px; box-shadow: 0 2px 6px rgba(0,0,0,0.05); }
.metric-box { background-color: #f1f3f5; padding: 12px; border-radius: 8px; margin-top: 10px; margin-bottom: 10px; font-size: 14px; }
.etf-metric-box { background-color: #fff5f5; padding: 12px; border-radius: 8px; margin-top: 10px; margin-bottom: 10px; border: 1px solid #ffe3e3; font-size: 14px; }
.biz-desc { font-size: 14px; color: #495057; background-color: #f8f9fa; padding: 10px; border-radius: 8px; margin-top: 8px; border-left: 3px solid #ced4da; line-height: 1.5; }
.advice-box { background-color: #fff9db; padding: 12px; border-radius: 8px; margin-top: 10px; border-left: 3px solid #fcc419; font-size: 14px; line-height: 1.5; }
.etf-advice-box { background-color: #fff0f6; padding: 12px; border-radius: 8px; margin-top: 10px; border-left: 3px solid #d6336c; font-size: 14px; line-height: 1.5; }
.adjust-box { background-color: #f1f8ff; border: 2px dashed #339af0; padding: 20px; border-radius: 12px; margin-bottom: 20px; }
...
def render_image_by_filename(filename, fallback_msg):
    candidate_files = [filename, filename.replace(".png", ".jpg"), filename.replace("_", "")]
    found = next((f for f in candidate_files if os.path.exists(f)), None)
    if found:
...
def recalculate_portfolio_mix(base_mix, safety_boost, dividend_boost, speed_boost):
    """유저의 세부 조율 입력을 받아 포트폴리오 자산 배분 비율을 동적 재계산하는 함수"""
...
if st.session_state.current_node != "END":
    ...
    if node == "Q1": render_three_pigs_crossroads_image()
    elif node == "Q2_A": render_image_by_filename("wolf_attack.png", "📌 늑대의 등장 이미지가 표시됩니다.")
    elif node == "Q2_B": render_image_by_filename("q2_b.png", "📌 정원 가꾸기 이미지가 표시됩니다.")
    elif node == "Q2_C": render_image_by_filename("q2_c.png", "📌 보물상자 이미지가 표시됩니다.")
    elif node == "Q3_A1": render_image_by_filename("q3_a1.png", "📌 동굴 속 돼지 이미지가 표시됩니다.")
    elif node == "Q3_A2": render_image_by_filename("q3_a2.png", "📌 월세 상자 이미지가 표시됩니다.")
    elif node == "Q3_B1": render_image_by_filename("q3_b1.png", "📌 거목의 조건 이미지가 표시됩니다.")
    elif node == "Q3_B2": render_image_by_filename("q3_b2.png", "📌 트렌드 열매 이미지가 표시됩니다.")
    elif node == "Q3_C1": render_image_by_filename("q3_c1.png", "📌 파도 타기 이미지가 표시됩니다.")
    elif node == "Q3_C2": render_image_by_filename("q3_c2.png", "📌 마법 주문 이미지가 표시됩니다.")
    ...
    # ★ 세부 조율 입력을 바탕으로 포트폴리오 자산 배분 동적 재계산 실행 ★
